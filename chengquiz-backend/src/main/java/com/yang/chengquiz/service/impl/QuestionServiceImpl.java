package com.yang.chengquiz.service.impl;
...
import com.yang.chengquiz.constant.CommonConstant;
import com.yang.chengquiz.exception.ThrowUtils;
import com.yang.chengquiz.mapper.QuestionMapper;
import com.yang.chengquiz.model.dto.question.QuestionContentDTO;
import com.yang.chengquiz.model.dto.question.QuestionQueryRequest;
...
import com.yang.chengquiz.model.vo.UserVO;
...
import com.yang.chengquiz.utils.SqlUtils;
...
public class QuestionServiceImpl extends ServiceImpl<QuestionMapper, Question> implements QuestionService {
     ...
     */
    @Override
    public void validQuestionContentDTO(List<QuestionContentDTO> questionContentDTOs, boolean add) {
        ThrowUtils.throwIf(questionContentDTOs == null, ErrorCode.PARAMS_ERROR);
        for(QuestionContentDTO questionContentDTO:questionContentDTOs){
            ...
            if (add) {
                // 补充校验规则
                ThrowUtils.throwIf(StringUtils.isBlank(title), ErrorCode.PARAMS_ERROR,"题目不能为空");
                ThrowUtils.throwIf(options==null||options.size()<2, ErrorCode.PARAMS_ERROR,"题目选项不能少于两个");
            }
            // 修改数据时，有参数则校验
            // 补充校验规则
            for(QuestionContentDTO.Option option:options){
                option.ValidOption(option,true);
            }
        }
    }
