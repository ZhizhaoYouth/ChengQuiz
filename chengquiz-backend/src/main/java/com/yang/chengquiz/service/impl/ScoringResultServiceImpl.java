...
import com.yang.chengquiz.mapper.ScoringResultMapper;
import com.yang.chengquiz.model.dto.scoringResult.ScoringResultQueryRequest;
...
import com.yang.chengquiz.model.vo.ScoringResultVO;
...
public class ScoringResultServiceImpl extends ServiceImpl<ScoringResultMapper, ScoringResult> implements ScoringResultService {
    ...
    public void validScoringResult(ScoringResult scoringResult, boolean add) {
        ...
        if (add) {
            ...
            ThrowUtils.throwIf(appType == 0 && (resultScoreRange == null || resultScoreRange <= 0), ErrorCode.PARAMS_ERROR, "结果分数范围必须大于0");
            ThrowUtils.throwIf(appType == 1 && resultProp == null, ErrorCode.PARAMS_ERROR, "结果属性不能为空");
            ThrowUtils.throwIf(appId == null || appId <= 0, ErrorCode.PARAMS_ERROR, "应用id非法");
        }
        // 修改数据时，有参数则校验
        // 补充校验规则
        if (StringUtils.isNotBlank(resultName)) {
            ThrowUtils.throwIf(resultName.length() > 128, ErrorCode.PARAMS_ERROR, "结果名称最多128个字符");
        }
    }
